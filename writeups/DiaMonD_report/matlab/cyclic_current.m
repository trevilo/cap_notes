...
for i = 75 : 150
    eta_bar(i) = tau(i)*I(i)+0.14+0.0098;
end
for i = 151 : 224
    eta_bar(i) = tau(i)*I(i)-0.1540;
end

for i = 225 : 300
    eta_bar(i) = tau(i)*I(i)+0.3006;
end

figure; plot(tau,eta_bar)
%%
...
plot(tau,etaLF(Nx*0.5,:),'LineWidth',3);
plot(tau,etaLF(Nx*0.75,:),'LineWidth',3);
...
plot(tau,etaHF(Nx*0.5,:),'LineWidth',3);
plot(tau,etaHF(Nx*0.75,:),'LineWidth',3);
...
epsilon0 = epsilon(1,:);
epsilon1 = epsilon(end,:);

error_estimate_Velec = ((1+2*gamma)/(1+gamma)) * epsilon1 - ...
    (gamma/(1+gamma))*epsilon0;
% error_estimate_Velec = (gamma*(gamma-2)/(6*(1+gamma)^2))*I;


figure
plot(tau,error_exact_Velec,'--','LineWidth',3); hold on
plot(tau,error_exact_Velec*0.95,'LineWidth',3); hold on
legend('exact', 'estimated')
axis square
set(gca,'LineWidth',2,'FontSize',24,'FontWeight','normal','FontName','Times')
set(get(gca,'XLabel'),'String','\tau','FontSize',42,'FontWeight','bold','FontName','Times')
set(get(gca,'YLabel'),'String','Error V^*_{electrod}','FontSize',42,'FontWeight','bold','FontName','Times')
set(get(gca,'Title'),'String','Constant Current','FontSize',32,'FontWeight','bold','FontName','Times')
set(gcf,'Position',[1 1 round(1000) round(1000)])

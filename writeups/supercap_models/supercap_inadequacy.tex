...
%================
\textit{Solution of LF model:}
Figures (\ref{fig:etaLF_cycl}) show the variation of solution of LF model (\ref{eq:LF}) in the normalized time and space. % Figure (\ref{fig:I_cycl}b) shows $\eta_{LF}^{avg}$, solution of (\ref{eq:LF_avg}) such as
%
\begin{equation}\label{eq:Iprim}
{\eta_{LF}^{avg}}(\tau) = \int_0^{\tau} I^*(\tau') d\tau' 
\end{equation}

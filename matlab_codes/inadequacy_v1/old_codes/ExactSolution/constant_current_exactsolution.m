...
% solving HF pde+bc from the analytical solution
...
for i = 1 : Ntau
    for j = 1 : Nxi
        ...
        S = symsum((((-1)^n+gamma)/n^2)*cos(n*pi*xi(j))*exp(-(n)^2*pi^2*tau(i)), n, 1, 10);
        etaHFexact(j,i) = I(i)*tau(i) + I(i)*((3*xi(j)^2-1)/(6*(1+gamma))) + I(i)*(gamma*(3*xi(j)^2-6*xi(j)+2)/(6*(1+gamma))) - ...
            2*I(i)/(pi^2*(1+gamma)) * S;
        
    end
end

etaHF = double(etaHFexact);
